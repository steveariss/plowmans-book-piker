...
Book data itself lives in `data/books.json` (static, generated by scraper).
...
1. **Project Scaffolding** — Initialize all package.json files, Vite, Express, SQLite schema, ESLint/Prettier, .gitignore, base CSS
2. **Scraper** — API discovery via Playwright, book list pagination, detail page scraping, image download with resumability
3. **Backend API** — All REST endpoints, DB init, error handling, static file serving
4. **Frontend Core** — Router, TeacherSetup, BookBrowsing grid, hooks, PickButton, SelectionCounter, DoneButton, ThankYou
5. **Frontend Carousel** — Book preview modal using `<dialog>`, swipeable image carousel, PickButton inside carousel
6. **Frontend Admin** — ManageBooks (virtualized grid, search, select all, delete/restore), Report (table, CSV download, clear data)
7. **Polish & Deploy** — Font loading, responsive testing at 1366×768, touch targets ≥ 48px, performance testing, deployment
